...
[xm,ym,zm]=Nodes3D(m); [rm,sm,tm]=xyztorst(xm,ym,zm);
Pq2 = Vandermonde3D(m,rm,sm,tm)*Vandermonde3D(m,rq,sq,tq)'*diag(wq);
invV1 = inv(Vandermonde3D(m,rm,sm,tm));

V1 =Vandermonde3D(1,rq,sq,tq);
Pq1 = V1 * V1' *diag(wq);
...
c_BB = zeros(size(Cq,1),K);
for i =1:K
    k = mD(i);
    v = C_modal(:,i);
    if(k==0)
        c_BB(:,i)=v(1)*ones(size(Cq,1),1);
    end
    if(k==1)
        c_BB(:,i)= Pq1*Cq(:,i);
    end
...
while (time<FinalTime)
    ...
    for INTRK = 1:5
        ...
        [rhsp, rhsu1, rhsu2, rhsu3] = acousticsRHS3D(p,u1,u2,u3);
        [rhsq, rhsv1, rhsv2, rhsv3] = acousticsRHS3D(q,v1,v2,v3);
