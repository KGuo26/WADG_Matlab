function [rhsp, rhsu, rhsv] = acousticsRHS2D_BBprojection(p,u,v,time)
...
divU = (T*(-0.5)*D0*uB).*rx + (T*0.5*D1*uB).*rx + (T*(-0.5)*D0*uB).*sx + (T*0.5*D2*uB).*sx...
      +(T*(-0.5)*D0*vB).*ry + (T*0.5*D1*vB).*ry + (T*(-0.5)*D0*vB).*sy + (T*0.5*D2*vB).*sy;
...
rhsp =  -divU; %+ LIFT1*inv(T)*(Fscale.*fluxp)/2.0;
rhsu =  -dpdx; %+ LIFT1*inv(T)*(Fscale.*fluxu)/2.0;
rhsv =  -dpdy; %+ LIFT1*inv(T)*(Fscale.*fluxv)/2.0;

[T0,T1,T2] = Vandermonde_BB_surface(r,s);

rhsp = rhsp + (T*LIFT1)* [inv(T0)*(Fscale(1:5,:).*fluxp(1:5,:));inv(T1)*(Fscale(6:10,:).*fluxp(6:10,:));inv(T2)*(Fscale(11:15,:).*fluxp(11:15,:))]/2.0;;
...
% Now, the rhsp is based on nodal basis.

% transform the rhsp into BB basis.
rhsp_BB = inv(T)*rhsp;

wu = zeros(15,512);

P = projection_BB_2D(4,0);

for j=1:512

    wu(:,j) = mult_BB_2D([1],rhsp_BB(:,j),0,4);
